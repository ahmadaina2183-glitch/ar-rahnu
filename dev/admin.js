...
function openTab(tab) { currentAdminTab = tab; document.querySelectorAll('.admin-tab').forEach((b) => b.classList.toggle('active', b.dataset.tab === tab)); document.querySelectorAll('.admin-panel').forEach((p) => p.classList.toggle('active', p.id === `admin-${tab}`)); $('adminBackBtn').hidden = tab === 'overview'; $('adminTitle').textContent = { overview: 'Admin Panel', staff: 'Manage Staff', customers: 'Manage Customer', settings: 'Settings', reports: 'Report' }[tab] || 'Admin Panel'; }
...
$('adminBackBtn')?.addEventListener('click', () => currentAdminTab === 'overview' ? history.back() : openTab('overview'));
